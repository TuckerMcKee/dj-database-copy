...
def show_song(song_id):
    ...
    song = Song.query.get_or_404(Song, song_id)
...
def add_song_to_playlist(playlist_id):
    ...
    songs = db.session.query(Song).join(PlaylistSong).filter(PlaylistSong.playlist_id == playlist_id).all()
    curr_on_playlist = [s.id for s in songs]
    form.song.choices = [
        (song.id, song.title) for song in db.session.query(Song).filter(Song.id.notin_(curr_on_playlist)).all()
    ]
    
    if form.validate_on_submit():
        song_id = form.song.data
        new_playlistsong = PlaylistSong(song_id=song_id,playlist_id=playlist_id)
